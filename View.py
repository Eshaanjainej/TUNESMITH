...
class View:
    ...
    def setup_player(self):
        ...
        self.top.title("TUNESMITH-Dance to the rhythm of your heart")
        img=tk.PhotoImage(file="D:\\tunesmith\icons\\3d-social-media-icons-music-free-png.png")
    ...
    def add_song(self):
        ...
        self.playList.config(fg=mycolors)
    ...
    def load_previous_song(self):
        try:
            if hasattr(self,"sel_song_index_tuple")==False:
                raise NoSongsSelectedError("Please play a Song First")
    ...
    def close_window(self):
        ...
        if result:
            ...
            messagebox.showinfo("Have a good day","Thank you for using TUNESMITH")
            self.top.destroy()
    ...
    def add_song_to_favourites(self):
        ...
        try:
            if(len(fav_song_index_tuple)==0):
                raise NoSongsSelectedError("Please select a song for adding to favourites")
    ...
    def remove_song_from_favourites(self):
        ...
        try:
            if (len(fav_song_index_tuple)==0):
                raise NoSongsSelectedError("Please select a song for removing from favourites")
            song_name=self.playList.get(fav_song_index_tuple)
            result=self.player.remove_song_from_favourite(song_name)
            if(result.find("deleted")!=-1):
                self.playList.delete(fav_song_index_tuple[0])
                if(fav_song_index_tuple[0]==self.sel_index_tuple[0]):
                    self.stop_song()
    ...
    def load_songs_from_favourites(self):
        try:
            ...
            mycolors = "#" + red + green + blue
            self.playList.config(fg=mycolors)
            messagebox.showinfo("Show Added!!",result_str)
        except (DatabaseError) as ex:
            messagebox.showinfo("DB error",ex)
